...
<div class="row">
      <!-- Default box -->
    <div class="col-md-6">
	<?php echo form_open_multipart('admin/Master/save-product-module', array('id' => 'master_frrm','class'=>'form-horizontal adminex-form' ,'enctype'=>'multipart/form-data')); ?>
	  <div class="card"> 	   
        ...
        <div class="card-body">
           <div class="form-group row">
                    <label for="inputEmail3" class="col-sm-4 col-form-label">  Product Module <label style="color:#FF0000">*</label></label>
                    <div class="col-sm-8">
						<input data-id="<?php echo @$result->id?>"   type="text" maxlength="250" class="form-control required no_special   " name="module_name" id="module_name" value="<?php echo @$result->module_name?>">
				   </div>
                </div>
			<div class="form-group row">
        ...
        <div class="card-footer">
			...
			<?php if($result->id){
				if($this->Home_model->check_permission('save_product_module')){?>
			<input type="button" name="save" id="save" value="<?php echo $this->lang->line('common_save_btn');?>" class="btn btn-info btn_validator" data-frm="master_frrm"    style="<?php if(@$result->id){?> width:50%; <?php }else{ ?> width:100%; <?php  } ?>float:left" />
				<?php }}else{?>
			<?php if($this->Home_model->check_permission('save_product_module')){?>
			<input type="button" name="save" id="save" value="<?php echo $this->lang->line('common_save_btn');?>" class="btn btn-info btn_validator" data-frm="master_frrm"    style="<?php if(@$result->id){?> width:50%; <?php }else{ ?> width:100%; <?php  } ?>float:left" />
			<?php } } ?>
			
			<?php if(@$result->id){?>
			<a style="float:right;width:40%" class="btn btn-secondary" href="Master/product-module-list"><?php echo $this->lang->line('common_close_btn');?></a>
			<?php } ?>
        </div>
        <!-- /.card-footer-->
      </div>
	  ...
	  <div class="col-md-6">
	<?php echo form_open_multipart('admin/Master/product-module-list', array('id' => 'basic_search','class'=>'form-horizontal adminex-form' ,'enctype'=>'multipart/form-data')); ?>
	  <div class="card"> 	   
        <div class="card-header">
          <h3 class="card-title">Search Product Module</h3>
        ...
        <div class="card-body">
           <div class="form-group row">
                    <label for="inputEmail3" class="col-sm-4 col-form-label">By Product Module </label>
                    <div class="col-sm-8">
		...
		<div class="form-group row">
			...
			<div class="col-sm-6">
				<a style="float:right;width:100%" class="btn btn-success" href="Master/product-module-list"><?php echo $this->lang->line('common_reset_btn');?></a>
			</div>
			</div>
        </div>
        <!-- /.card-footer-->
...
<?php if($this->Home_model->check_permission('product_module_list_export')){?>
<div class="col-sm-2" style="float:right;"> 
		...
		<?php if($this->Home_model->check_permission('product_module_list')){?>
        <div class="card-body">
           <table id="example_php" class="table table-bordered table-striped col-xs-12" >
								<thead>
									<tr>
										<th>#</th>
										<th>  Module Name
 <i style="cursor:pointer" class="fas fa-arrow-circle-up sorting_btn"  data-frm="basic_search" data-filed="module_name" data-type="ASC"></i>
 <i style="cursor:pointer" class="fas fa-arrow-circle-down sorting_btn"  data-frm="basic_search" data-filed="module_name" data-type="DESC"></i>
   </th>
								...
								<tbody>
									...
									<tr>
										...
										<td><input type="checkbox" value="1"  data-msg="<?php echo $this->lang->line('admin_msg_status_updated');?>" data-confirm-msg="<?php echo $this->lang->line('admin_msg_status_update_confirm');?>"  class=" <?php if($this->Home_model->check_permission('save_product_module')){?> update_status <?php } ?> " data-id="<?php echo $result->id?>" data-module="product_module_master"  data-filed="is_active" <?php if(@$result->is_active){?> checked <?php } ?>></td>
										<td>
										<?php if($this->Home_model->check_permission('save_product_module')){?> 
										
										<a class="btn btn-xs btn-info" href="master/product-module-list/<?php echo $result->id?>" title="Edit"><i class="fas fa-pencil-alt "></i></a>
										<?php }?>
										<?php  
										if($this->Home_model->check_permission('product_module_delete')){?>
										<a class="btn btn-xs btn-danger confirm_delete" href="master/product-module-delete/<?php echo $result->id?>" title="Delete"><i class="fa fa-trash"></i></a>
										<?php }?>
										</td>
									</tr>
									<?php } ?>
