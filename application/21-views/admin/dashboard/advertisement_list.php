...
<div class="row">
 ...
 <i style="cursor:pointer" class="fas fa-arrow-circle-down sorting_btn"  data-frm="basic_search" data-filed="name" data-type="DESC"></i>
										...
										<th><?php echo $this->lang->line('admin_advertisement_from_date_label');?> </th>  
										<th><?php echo $this->lang->line('admin_advertisement_to_date_label');?> </th>  
...
<th><?php echo $this->lang->line('common_ispublished_label');?> </th>
								...
								<tbody>
									...
									<tr>
										...
										<td><input type="checkbox" value="1" data-msg="<?php echo $this->lang->line('admin_msg_status_updated');?>" data-confirm-msg="<?php echo $this->lang->line('admin_msg_status_update_confirm');?>" class="<?php if($this->Home_model->check_permission('master_management','edit')){ ?> update_status <?php } ?>" data-id="<?php echo $result->id?>" data-module="advertisement"  data-filed="is_published" <?php if(@$result->is_published){?> checked <?php } ?>></td>
										
										<td>
										<?php if($this->Home_model->check_permission('master_management','edit')){ ?>
										<a class="btn btn-xs btn-info" href="Home/advertisement-add/<?php echo $result->id?>" title="Edit"><i class="fas fa-pencil-alt "></i></a>
										<?php } if($this->Home_model->check_permission('master_management','delete')){ ?>
										<a class="btn btn-xs btn-danger confirm_delete" href="Home/advertisement-delete/<?php echo $result->id?>" title="Delete"><i class="fa fa-trash"></i></a>
										<?php } ?>
										</td>
									</tr>
									<?php } ?>
