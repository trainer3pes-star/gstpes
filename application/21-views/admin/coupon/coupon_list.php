...
<div class="row">
     ...
     <div class="col-md-2"></div>
	  <div class="col-md-8">
	<?php echo form_open_multipart('admin/master/coupon-list', array('id' => 'basic_search','class'=>'form-horizontal adminex-form' ,'enctype'=>'multipart/form-data')); ?>
	  <div class="card"> 	   
        <div class="card-header">
          <h3 class="card-title">Search Coupon</h3>
        ...
        <div class="card-body">
           <div class="form-group row"> 
				   ...
				   <div class="col-sm-4">
					<label for="inputEmail3" class="col-sm-12 col-form-label">Coupon For</label>
					 <select name="coupon_for" id="coupon_for" class=" form-control select2 " style="width:100%;"> 
					 <option value="0"  ><?php echo $this->lang->line('common_all');?></option>
						<option value="1" <?php if(@$search['coupon_for']==1){ ?> selected <?php } ?>> All Category</option> 
						<option value="2" <?php if(@$search['coupon_for']==2){ ?> selected <?php } ?>> Specific Categories</option> 
						<option value="3" <?php if(@$search['coupon_for']==3){ ?> selected <?php } ?>> Specific Products</option> 
						</select> 
			...
			<div class="col-sm-4"> 
				<a style="float:right;width:100%" class="btn btn-success" href="master/coupon-list"><?php echo $this->lang->line('common_reset_btn');?></a>
			</div>
			<?php if($this->Home_model->check_permission('1','PAdd')){?>
			<div class="col-sm-4"> 
				<a style="float:right;width:100%" class="btn btn-secondary" href="master/create-coupon">Create Coupon</a>
			</div>
			<?php } ?>
                </div>
        </div>
 ...
 <i style="cursor:pointer" class="fas fa-arrow-circle-down sorting_btn"  data-frm="basic_search" data-filed="CreatedDate" data-type="DESC"></i>
								...
								<tbody>
									...
									<tr  >
										...
										<td><?php echo $result->coupon_title;?></td> 
										...
										<td><?php if($result->coupon_for==1){ echo "All Category";}elseif($result->coupon_for==2){ echo "Specific Categories"; }else{ echo "Specific Products";}?></td>    
										...
										<td><input type="checkbox" value="1"  data-msg="<?php echo $this->lang->line('admin_msg_status_updated');?>" data-confirm-msg="<?php echo $this->lang->line('admin_msg_status_update_confirm');?>"  class=" update_status " data-id="<?php echo $result->id?>" data-module="coupon_slider"  data-filed="is_active" <?php if(@$result->is_active){?> checked <?php } ?>></td>
										<td> 
										
										<?php if($this->Home_model->check_permission('1','PEdit')){?> 
										<a class="btn btn-xs btn-info" href="master/create-coupon/<?php echo $result->id?>" title="Edit"><i class="fas fa-pencil-alt "></i></a>
										<?php } ?>
										 
										<?php if($this->Home_model->check_permission('1','PDelete')){?>	
										<a class="btn btn-xs btn-danger confirm_delete" href="master/delete-coupon/<?php echo $result->id?>" title="Delete"><i class="fa fa-trash"></i></a>
										<?php } ?> 
										</td> 
