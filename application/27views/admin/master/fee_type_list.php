...
<div class="row">
      <!-- Default box -->
	  <?php if($this->Home_model->check_permission('save_fee_type')){?>
    <div class="col-md-6">
	<?php echo form_open_multipart('admin/Master/save-fee-type', array('id' => 'master_frrm','class'=>'form-horizontal adminex-form' ,'enctype'=>'multipart/form-data')); ?>
	  <div class="card"> 	   
        ...
        <div class="card-body">
           <div class="form-group row">
                    <label for="inputEmail3" class="col-sm-4 col-form-label">  Fee Type <label style="color:#FF0000">*</label></label>
                    <div class="col-sm-8">
						<input data-id="<?php echo @$result->id?>"   type="text" maxlength="250" class="form-control required no_special   " name="name" id="name" value="<?php echo @$result->name?>">
				   </div>
                </div>
			<div class="form-group row">
        ...
        <div class="card-footer">
			...
			<?php if($result->id){
				if($this->Home_model->check_permission('save_fee_type')){?>
			...
			<?php if(@$result->id){?>
			<a style="float:right;width:40%" class="btn btn-secondary" href="Master/fee-type-list"><?php echo $this->lang->line('common_close_btn');?></a>
			<?php } ?>
        </div>
        <!-- /.card-footer-->
      </div>
	  ...
	  <?php } ?>  
	  <div class="col-md-6">
	<?php echo form_open_multipart('admin/Master/fee-type-list', array('id' => 'basic_search','class'=>'form-horizontal adminex-form' ,'enctype'=>'multipart/form-data')); ?>
	  <div class="card"> 	   
        <div class="card-header">
          <h3 class="card-title">Search Fee Type</h3>
        ...
        <div class="card-body">
           <div class="form-group row">
                    <label for="inputEmail3" class="col-sm-4 col-form-label">By Fee Type </label>
                    <div class="col-sm-8">
		...
		<div class="form-group row">
			...
			<div class="col-sm-6">
				<a style="float:right;width:100%" class="btn btn-success" href="Master/fee-type-list"><?php echo $this->lang->line('common_reset_btn');?></a>
			</div>
			</div>
        </div>
        <!-- /.card-footer-->
...
<?php if($this->Home_model->check_permission('fee_type_list_export')){?>
<div class="col-sm-2" style="float:right;"> 
		...
		<?php if($this->Home_model->check_permission('fee_type_list')){?>
        <div class="card-body">
 ...
 <i style="cursor:pointer" class="fas fa-arrow-circle-down sorting_btn"  data-frm="basic_search" data-filed="TypeName" data-type="DESC"></i>
								...
								<tbody>
									...
									<tr>
										...
										<td><input type="checkbox" value="1"  data-msg="<?php echo $this->lang->line('admin_msg_status_updated');?>" data-confirm-msg="<?php echo $this->lang->line('admin_msg_status_update_confirm');?>"  class=" <?php if($this->Home_model->check_permission('save_fee_type')){?> update_status <?php } ?> " data-id="<?php echo $result->id?>" data-module="user_role_master"  data-filed="is_active" <?php if(@$result->is_active){?> checked <?php } ?>></td>
										<td>
										<?php if($this->Home_model->check_permission('save_fee_type')){?> 
										
										<a class="btn btn-xs btn-info" href="master/fee-type-list/<?php echo $result->id?>" title="Edit"><i class="fas fa-pencil-alt "></i></a>
										<?php }?>
										<?php  
										if($this->Home_model->check_permission('fee_type_delete')){?>
										<a class="btn btn-xs btn-danger confirm_delete" href="master/fee-type-delete/<?php echo $result->id?>" title="Delete"><i class="fa fa-trash"></i></a>
										<?php }?>
										</td>
									</tr>
									<?php } ?>
