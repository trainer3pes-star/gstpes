...
<div class="row">
      <!-- Default box -->
	  <?php if($this->Home_model->check_permission('save_user_role')){?>
    <div class="col-md-6">
	<?php echo form_open_multipart('admin/Master/save-user-role', array('id' => 'master_frrm','class'=>'form-horizontal adminex-form' ,'enctype'=>'multipart/form-data')); ?>
	  <div class="card"> 	   
        ...
        <div class="card-body">
           <div class="form-group row">
                    <label for="inputEmail3" class="col-sm-4 col-form-label">Role Name <label style="color:#FF0000">*</label></label>
                    <div class="col-sm-8">
						<input data-id="<?php echo @$result->id?>" data-module="User_Type" type="text" maxlength="250" class="form-control required no_special   " name="name" id="name" value="<?php echo @$result->name?>">
				   </div>
                </div>
			<div class="form-group row">
        ...
        <div class="card-footer">
			...
			<?php if($result->id){
				if($this->Home_model->check_permission('save_user_role')){?>
			...
			<?php if(@$result->id){?>
			<a style="float:right;width:40%" class="btn btn-secondary" href="Master/user-role-list"><?php echo $this->lang->line('common_close_btn');?></a>
			<?php } ?>
        </div>
        <!-- /.card-footer-->
      </div>
	  ...
	  <?php  } ?>
	   <?php if($this->Home_model->check_permission('user_role_list')){?>
	  <div class="col-md-6">
	<?php echo form_open_multipart('admin/Master/user-role-list', array('id' => 'basic_search','class'=>'form-horizontal adminex-form' ,'enctype'=>'multipart/form-data')); ?>
	  <div class="card"> 	   
        <div class="card-header">
          <h3 class="card-title">Search User Role</h3>
        ...
        <div class="card-body">
           <div class="form-group row">
                    <label for="inputEmail3" class="col-sm-4 col-form-label">By Role </label>
                    <div class="col-sm-8">
		...
		<div class="form-group row">
			...
			<div class="col-sm-6">
				<a style="float:right;width:100%" class="btn btn-success" href="Master/user-role-list"><?php echo $this->lang->line('common_reset_btn');?></a>
			</div>
			</div>
        </div>
        <!-- /.card-footer-->
...
<?php if($this->Home_model->check_permission('user_role_list_export')){?>
           ...
           <table id="example_php" class="table table-bordered table-striped col-xs-12" >
								<thead>
									<tr>
										<th>#</th>
										<th>Role Name
 <i style="cursor:pointer" class="fas fa-arrow-circle-up sorting_btn"  data-frm="basic_search" data-filed="TypeName" data-type="ASC"></i>
 <i style="cursor:pointer" class="fas fa-arrow-circle-down sorting_btn"  data-frm="basic_search" data-filed="TypeName" data-type="DESC"></i>
   </th>
								...
								<tbody>
									...
									<tr>
										...
										<td><input type="checkbox" value="1"  data-msg="<?php echo $this->lang->line('admin_msg_status_updated');?>" data-confirm-msg="<?php echo $this->lang->line('admin_msg_status_update_confirm');?>"  class=" <?php if($this->Home_model->check_permission('2','PEdit')){?> update_status <?php } ?> " data-id="<?php echo $result->id?>" data-module="user_role_master"  data-filed="is_active" <?php if(@$result->is_active){?> checked <?php } ?>></td>
										<td>
										<?php if($this->Home_model->check_permission('save_user_role')){?> 
										
										<a class="btn btn-xs btn-info" href="master/user-role-list/<?php echo $result->id?>" title="Edit"><i class="fas fa-pencil-alt "></i></a>
										<?php }?>
										<?php 
										if($result->is_delete==1){
										if($this->Home_model->check_permission('user_role_delete')){?>
										<a class="btn btn-xs btn-danger confirm_delete" href="master/user-role-delete/<?php echo $result->id?>" title="Delete"><i class="fa fa-trash"></i></a>
										<?php }}?>
										</td>
									</tr>
									<?php } ?>
