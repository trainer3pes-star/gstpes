...
<div class="row">
     ...
     <div class="col-md-2"></div>
	  <div class="col-md-8">
	<?php echo form_open_multipart('admin/User/admin-user-list', array('id' => 'basic_search','class'=>'form-horizontal adminex-form' ,'enctype'=>'multipart/form-data')); ?>
	  <div class="card"> 	   
        ...
        <div class="card-body">
           <div class="form-group row"> 
			...
			<div class="col-sm-4"> 
				<a style="float:right;width:100%" class="btn btn-success" href="User/admin-user-list"><?php echo $this->lang->line('common_reset_btn');?></a>
			</div>
			<?php if($this->Home_model->check_permission('1','PAdd')){?>
			<div class="col-sm-4"> 
				<a style="float:right;width:100%" class="btn btn-secondary" href="User/create-admin-user">Add User</a>
			</div>
			<?php } ?>
                </div>
        </div>
 ...
 <i style="cursor:pointer" class="fas fa-arrow-circle-down sorting_btn"  data-frm="basic_search" data-filed="CreatedDate" data-type="DESC"></i>
								...
								<tbody>
									...
									<tr  >
										...
										<td> 
										<?php $a_pers=$this->User_model->get_assign_user_permission_list($result->id);?>
										 <div class="btn-group">
                    <button type="button" class="btn bg-fuchsia ">Permissions</button>
                    <button type="button" class="btn bg-fuchsia  dropdown-toggle  dropdown-icon" data-toggle="dropdown">
                      <span class="sr-only">Toggle Dropdown</span>
                      <ul class="dropdown-menu" role="menu"style="overflow-y: auto !important;max-height: 100px !important;">
						<?php foreach($a_pers as $a_per){?>
                        <li class="dropdown-item" ><?php echo $a_per->display_name?></li> 
						<?php } ?>
                      </ul>
                    </button>
                  </div>
										<?php if($this->Home_model->check_permission('1','PEdit')){?> 
										<a class="btn btn-xs btn-info" href="User/create-admin-user/<?php echo $result->id?>" title="Edit"><i class="fas fa-pencil-alt "></i></a>
										<?php } ?>
										 
										<?php if($this->Home_model->check_permission('1','PDelete')){?>	
										<a class="btn btn-xs btn-danger confirm_delete" href="User/delete-admin-user/<?php echo $result->id?>" title="Delete User"><i class="fa fa-trash"></i></a>
										<?php } ?>
										</td> 
