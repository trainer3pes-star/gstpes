...
<div class="row">
     ...
     <div class="col-md-2"></div>
	  <div class="col-md-8">
	<?php echo form_open_multipart('admin/product/product-category-list', array('id' => 'basic_search','class'=>'form-horizontal adminex-form' ,'enctype'=>'multipart/form-data')); ?>
	  <div class="card"> 	   
        <div class="card-header">
          <h3 class="card-title">Search Product Category</h3>
        ...
        <div class="card-body">
           <div class="form-group row"> 
                    <div class="col-sm-6">
					<label for="inputEmail3" class="col-sm-12 col-form-label">Category Name </label>
				   ...
				   <div class="col-sm-4">
					<label for="inputEmail3" class="col-sm-12 col-form-label">Parent Category</label>
			...
			<div class="col-sm-4"> 
				<a style="float:right;width:100%" class="btn btn-success" href="product/product-category-list"><?php echo $this->lang->line('common_reset_btn');?></a>
			</div>
			<?php if($this->Home_model->check_permission('1','PAdd')){?>
			<div class="col-sm-4"> 
				<a style="float:right;width:100%" class="btn btn-secondary" href="product/create-product-category">Create category</a>
			</div>
			<?php } ?>
                </div>
        </div>
 ...
 <i style="cursor:pointer" class="fas fa-arrow-circle-down sorting_btn"  data-frm="basic_search" data-filed="CreatedDate" data-type="DESC"></i>
								...
								<tbody>
									...
									<tr  >
										...
										<td><?php echo $result->parent_category_name;?></td>  
										<td><?php echo $result->short_notes;?></td>  
										
										
										<td><?php echo $result->seller_standard_margin;?>%</td>   
										<td><input type="checkbox" value="1"  data-msg="Update on home page " data-confirm-msg="Are you sure do you want to update on home page"  class=" update_status " data-id="<?php echo $result->id?>" data-module="category"  data-filed="home_page_category" <?php if(@$result->home_page_category){?> checked <?php } ?>></td>
										<td><?php echo date(DISPLAY_DATE,strtotime($result->created_date));?></td> 
										<td><input type="checkbox" value="1"  data-msg="<?php echo $this->lang->line('admin_msg_status_updated');?>" data-confirm-msg="<?php echo $this->lang->line('admin_msg_status_update_confirm');?>"  class=" update_status " data-id="<?php echo $result->id?>" data-module="category"  data-filed="is_active" <?php if(@$result->is_active){?> checked <?php } ?>></td>
										<td> 
										
										<?php if($this->Home_model->check_permission('1','PEdit')){?> 
										<a class="btn btn-xs btn-info" href="product/create-product-category/<?php echo $result->id?>" title="Edit"><i class="fas fa-pencil-alt "></i></a>
										<?php } ?>
										 
										<?php if($this->Home_model->check_permission('1','PDelete')){?>	
										<a class="btn btn-xs btn-danger confirm_delete" href="product/delete-product-category/<?php echo $result->id?>" title="Delete"><i class="fa fa-trash"></i></a>
										<?php } ?> 
										</td> 
