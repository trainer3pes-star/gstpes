...
<div class="row">
     ...
     <div class="col-md-2"></div>
	  <div class="col-md-8">
	<?php echo form_open_multipart('admin/home/banner-list', array('id' => 'basic_search','class'=>'form-horizontal adminex-form' ,'enctype'=>'multipart/form-data')); ?>
	  <div class="card"> 	   
        ...
        <div class="card-body">
           <div class="form-group row"> 
                    <div class="col-sm-6">
					<label for="inputEmail3" class="col-sm-12 col-form-label">Banner Heading / Info</label>
				   ...
				   <div class="col-sm-4">
					<label for="inputEmail3" class="col-sm-12 col-form-label">Banner Type</label>
					 ...
					 <option value="0"  ><?php echo $this->lang->line('common_all');?></option>
						<option value="top_slider" <?php if(@$search['banner_type']=='top_slider'){?> selected <?php } ?>> Home Top Slider</option> 
						</select> 
			...
			<div class="col-sm-4"> 
				<a style="float:right;width:100%" class="btn btn-success" href="Home/banner-list"><?php echo $this->lang->line('common_reset_btn');?></a>
			</div>
			<?php if($this->Home_model->check_permission('1','PAdd')){?>
			<div class="col-sm-4"> 
				<a style="float:right;width:100%" class="btn btn-secondary" href="Home/create-banner">Create Banner</a>
			</div>
			<?php } ?>
                </div>
        </div>
 ...
 <i style="cursor:pointer" class="fas fa-arrow-circle-down sorting_btn"  data-frm="basic_search" data-filed="CreatedDate" data-type="DESC"></i>
								...
								<tbody>
									...
									<tr  >
										...
										<td><?php echo $result->banner_heading;?></td> 
										<td><?php echo $result->banner_info;?></td>  
										<td><?php echo $result->banner_link;?></td>  
										<td><?php echo ucwords(str_replace('_',' ',$result->banner_type));?></td>  
										<td><img src="<?php echo base_url(); ?>uploads/banner/thumb/<?php echo $result->banner_img;?>" style="width:100px;"></td>
										  
										<td><?php echo date(DISPLAY_DATE,strtotime($result->created_date));?></td> 
										<td><input type="checkbox" value="1"  data-msg="<?php echo $this->lang->line('admin_msg_status_updated');?>" data-confirm-msg="<?php echo $this->lang->line('admin_msg_status_update_confirm');?>"  class=" update_status " data-id="<?php echo $result->id?>" data-module="banner_slider"  data-filed="is_active" <?php if(@$result->is_active){?> checked <?php } ?>></td>
										<td> 
										
										<?php if($this->Home_model->check_permission('1','PEdit')){?> 
										<a class="btn btn-xs btn-info" href="Home/create-banner/<?php echo $result->id?>" title="Edit"><i class="fas fa-pencil-alt "></i></a>
										<?php } ?>
										 
										<?php if($this->Home_model->check_permission('1','PDelete')){?>	
										<a class="btn btn-xs btn-danger confirm_delete" href="Home/delete-banner/<?php echo $result->id?>" title="Delete"><i class="fa fa-trash"></i></a>
										<?php } ?> 
										</td> 
